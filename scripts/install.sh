#!/bin/sh
set -e
# Based on godownloader output; refactored to match yindia/pltf release asset names:
#   pltf_<version>_<os>_<arch>.tar.gz (mac/linux) and .zip (windows)
# and checksums.txt present in the release.

usage() {
  this=$1
  cat <<EOF
$this: download binaries for yindia/pltf

Usage: $this [-b] bindir [-d] [tag]
  -b sets bindir or installation directory, Defaults to ./bin
  -d turns on debug logging
   [tag] is a tag from
   https://github.com/yindia/pltf/releases
...
# pltf_0.0.1-beta_darwin_arm64.tar.gz etc.
