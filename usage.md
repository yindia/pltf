...
- Builds the Docker images declared in the spec via Dagger (no push), renders the workspace, and runs `terraform plan` locally inside `.pltf/<spec>/<env>/workspace`.
...
- Produces `.pltf-plan.tfplan` and optional plan JSON, which `apply`/`destroy` automatically reuse with `-auto-approve`.
- Example: `pltf terraform plan -f service.yaml -e dev --detailed-exitcode --plan-file=/tmp/plan.tfplan`

### `pltf terraform apply`

- Builds and pushes images (host registry auth managed on the host) via Dagger and runs `terraform apply -auto-approve` locally, using the workspace and plan generated by `plan`.
- Example: `pltf terraform apply -f env.yaml -e prod`

### `pltf terraform destroy`

- Skips image builds, reuses the generated workspace/plan, and runs `terraform destroy -auto-approve` locally so automation never pauses.
