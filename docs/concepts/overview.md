...
## Core specs
- **Stack**: Reusable module bundle (EKS + nodegroups + observability, etc.).
...
Services point at Environments via `metadata.ref` (path to env file). Module links let services consume environment outputs without hand-written interpolation.

## Loop
1. Author/update YAML specs (Stack + Environment + Service).
