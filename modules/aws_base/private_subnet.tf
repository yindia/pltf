...
resource "aws_eip" "nat_eips" {
  ...
  domain = "vpc"
  tags = {
    Name      = "pltf-${var.layer_name}-nat-ip-${data.aws_availability_zones.current.zone_ids[count.index]}"
    terraform = "true"
  }
}
...
resource "aws_nat_gateway" "nat_gateways" {
  ...
  tags = {
    Name      = "pltf-${var.layer_name}-${data.aws_availability_zones.current.zone_ids[count.index]}"
    terraform = "true"
  }
}
