...
func (a *Azure) RequiredProviders(body *hclwrite.Body, needsK8s bool, needsHelm bool, needsKustomize bool) {
	body.SetAttributeValue("azurerm", cty.ObjectVal(map[string]cty.Value{
		"source":  cty.StringVal("hashicorp/azurerm"),
		"version": cty.StringVal(provider.ResolveProviderVersion("azurerm")),
	}))
	if needsK8s {
...
func (a *Azure) Backend(body *hclwrite.Body, bucket, key, _, _, container, resourceGroup, workspaceKeyPrefix string) {
	if bucket == "" {
...
func (a *Azure) Provider(body *hclwrite.Body, _, subscriptionID string, useVarRefs bool) {
	provBlock := body.AppendNewBlock("provider", []string{"azurerm"})
	provBody := provBlock.Body()
	if useVarRefs {
		provBody.SetAttributeRaw("subscription_id", hclwrite.TokensForTraversal(hcl.Traversal{
