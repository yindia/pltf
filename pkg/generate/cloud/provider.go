...
type Provider interface {
	...
	RequiredProviders(body *hclwrite.Body, needsK8s bool, needsHelm bool, needsKustomize bool)
	// Backend returns the HCL block for the `backend` section in `versions.tf`.
	Backend(body *hclwrite.Body, bucket, key, region, profile, container, resourceGroup, workspaceKeyPrefix string)
	// Provider returns the HCL block for the `provider` section in `providers.tf`.
	Provider(body *hclwrite.Body, region, account string, useVarRefs bool)
}
